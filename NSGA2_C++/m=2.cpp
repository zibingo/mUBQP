/* 
* @Author: zibin 
* @Last Modified time: 2022-05-10 19:45:38  
...
public:
    ...
    Individual(vector<int> x, vector<vector<int> > q1, vector<vector<int> > q2)
    {
        f1 = 0;
        f2 = 0;
        X = x;
...
void Init(int N, vector<Individual> &P, int n, vector<vector<int> > q1, vector<vector<int> > q2, Logger &logger)
{
    for (int i = 0; i < N; i++)
    {
        vector<int> X;
        Create_Decision_Vector(n, X);
        P.push_back(Individual(X, q1, q2));
    }
    logger.INFO("成功初始化种群");
}
...
{
    bool flag = false;
    if (p1.f1 == p2.f1 && p1.f2 == p2.f2 && p1.nnd == p2.nnd && p1.paretorank == p2.paretorank && VectorEqual(p1.X, p2.X))
    {
        flag = true;
    }
    return flag;
...
Individual Cross_Mutation(Individual p1, Individual p2, float Pc, float Pm, vector<vector<int> > q1, vector<vector<int> > q2)
{
    int n = p1.X.size();
    vector<int> X;
...
Individual LocalSearch(Individual parent, int MaxDepth, vector<vector<int> > q1, vector<vector<int> > q2)
{
    ...
    while (depth < MaxDepth)
    {
        ...
        while (flipindex < temp.size())
        {
            //使用公式求得翻转前后差值
            vector<int> off_f;
            int f1 = curf.at(0) + DeltaC1[flipindex];
            int f2 = curf.at(1) + DeltaC2[flipindex];
            off_f.push_back(f1);
            off_f.push_back(f2);
    ...
    while (depth <= MaxDepth)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (JudgeTaboo(TaboList, i) == false)
            {
                ...
                else if (Dominate(off_f, tempf) == -1 && sqrt(pow(off_f1 - tempf.at(0), 2) + pow(off_f2 - tempf.at(1), 2)) < distance)
                {
                    distance = sqrt(pow(off_f1 - tempf.at(0), 2) + pow(off_f2 - tempf.at(1), 2));
                    flipindex = i;
                }
            }
            else
...
void Evolution(int Max_Gen, int SearchMethod, int SearchDepth, int N, vector<Individual> &P, map<int, vector<int> > F1, float Pc, float Pm, vector<vector<int> > q1, vector<vector<int> > q2, int t, Logger &logger)
{
    int gen = 1;
    while (gen <= Max_Gen)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            Individual p = Cross_Mutation(p1, p2, Pc, Pm, q1, q2);
            if (SearchMethod == 0)
                offspring.push_back(p);
            else if (SearchMethod == 1)
                offspring.push_back(LocalSearch(p, SearchDepth, q1, q2));
            else
                offspring.push_back(TabuSearch(p, SearchDepth, SearchDepth / 5, q1, q2));
        }
