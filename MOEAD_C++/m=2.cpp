/* 
* @Author: zibin 
* @Last Modified time: 2022-05-10 19:42:51  
...
void Return_Matrix(vector<vector<int> > Q, vector<vector<int> > &q1, vector<vector<int> > &q2)
{
    int n = sqrt(Q.size());
    for (int i = 0; i < n; i++)
    {
...
void Neighbor(vector<vector<double> > Weights, vector<vector<int> > &B, int T)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        for (int j = 0; j < N; j++)
        {
            long double d = sqrt(pow(Weights.at(i).at(0) - Weights.at(j).at(0), 2) + pow(Weights.at(i).at(1) - Weights.at(j).at(1), 2));
            v.push_back(d);
        }
...
void Matrix_X(vector<int> X, vector<vector<int> > q1, vector<vector<int> > q2, int &sum1, int &sum2)
{
    int n = X.size();
    for (int i = 0; i < n; i++)
    {
...
public:
    ...
    Individual(vector<int> x, vector<double> w, vector<vector<int> > q1, vector<vector<int> > q2)
    {
        //初始化f1,f2,这里不能删的
        f1 = 0;
        f2 = 0;
        X = x;
        W = w;
        Matrix_X(X, q1, q2, f1, f2);
        f.push_back(f1);
        f.push_back(f2);
        F = W.at(0) * f1 + W.at(1) * f2;
    }
    ~Individual()
    {
        vector<int>().swap(X);
...
void Init(int N, vector<Individual> &P, vector<vector<double> > Weights, int n, vector<vector<int> > q1, vector<vector<int> > q2, Logger &logger)
{
    for (int i = 0; i < N; i++)
    {
        vector<int> X;
        Create_Decision_Vector(n, X);
        P.push_back(Individual(X, Weights.at(i), q1, q2));
        vector<int>().swap(X);
    }
    logger.INFO("成功初始化种群");
}

Individual cross_mutation(Individual p1, Individual p2, vector<double> W, float Pc, float Pm, vector<vector<int> > q1, vector<vector<int> > q2)
{
    //--------------一个后代--------------
    int n = p1.X.size();
    vector<int> X;
...
void UpdateDomain(Individual p, int i, int T, vector<Individual> &P, vector<vector<int> > B)
{
    for (int j = 0; j < T; j++)
    {
        double F = P.at(B.at(i).at(j)).W.at(0) * p.f1 + P.at(B.at(i).at(j)).W.at(1) * p.f2;
...
void InitDeltaC(vector<int> X, vector<vector<int> > q1, vector<vector<int> > q2, vector<int> &DeltaC1, vector<int> &DeltaC2)
{
    int n = X.size();
    for (int j = 0; j < n; j++)
    {
...
void UpdateDeltaC(vector<int> X, int j, vector<vector<int> > q1, vector<vector<int> > q2, vector<int> &DeltaC1, vector<int> &DeltaC2)
{
    int n = X.size();
    for (int i = 0; i < n; i++)
    {
...
Individual TabuSearch(Individual parent, int MaxDepth, int Length, vector<vector<int> > q1, vector<vector<int> > q2)
{
    int n = parent.X.size();
    vector<int> TaboList, DeltaC1, DeltaC2;
    InitTaboList(TaboList, n);
    InitDeltaC(parent.X, q1, q2, DeltaC1, DeltaC2);
    ...
    while (depth <= MaxDepth)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            double offF = off_f1 * parent.W.at(0) + off_f2 * parent.W.at(1);
    ...
    return Individual(bestX, parent.W, q1, q2);
}
Individual LocalSearch(Individual parent, int MaxDepth, vector<vector<int> > q1, vector<vector<int> > q2)
{
    int depth = 0;
    ...
    while (depth < MaxDepth)
    {
        ...
        while (flipindex < temp.size())
        {
            ...
            double offF = f1 * parent.W.at(0) + f2 * parent.W.at(1);
            if (offF > F)
            {
                F = offF;
                f.at(0) = f1;
                ...
                bestX = temp;
                UpdateDeltaC(temp, flipindex, q1, q2, DeltaC1, DeltaC2);
                break;
            }
            flipindex++;
        }
...
void Evolution(int Max_Gen, int SearchMethod, int SearchDepth, vector<Individual> &P, vector<vector<int> > B, int N, int T, float Pc, float Pm, vector<Individual> &EP, vector<vector<int> > q1, vector<vector<int> > q2, int t, Logger &logger)
{
    int gen = 1;
    while (gen <= Max_Gen)
    {
        for (int i = 0; i < N; i++)
        {
            ...
            /* ------------------ 一个后代 ------------------*/
            Individual p, p1;
            p = cross_mutation(P.at(B.at(i).at(k)), P.at(B.at(i).at(l)), P.at(i).W, Pc, Pm, q1, q2);
            if (SearchMethod == 0)
            {
                UpdateEp(p, EP);
            ...
            {
                p1 = LocalSearch(p, SearchDepth, q1, q2);
                UpdateEp(p1, EP);
                UpdateDomain(p1, i, T, P, B);
            }
            else
            {
                p1 = TabuSearch(p, SearchDepth, SearchDepth / 5, q1, q2);
                UpdateEp(p1, EP);
                UpdateDomain(p1, i, T, P, B);
            }
        }
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        string QName = "../instances/mubqp_" + occ + "_2_" + to_string(n) + "_0.8_0.dat";
        string WName = "../instances/weight_m=2.txt";
        vector<vector<int> > Q, q1, q2, B;
        vector<vector<double> > Weights;
        vector<Individual> P, EP;
        Q.reserve(pow(n, 2));
        Read_Matrix_file(QName, Q, logger);
        Read_Weight(WName, Weights, logger);
        Return_Matrix(Q, q1, q2);
        int N = Weights.size();
        Neighbor(Weights, B, T);
        Init(N, P, Weights, n, q1, q2, logger);
        Evolution(Max_Gen, SearchMethod, SearchDepth, P, B, N, T, Pc, Pm, EP, q1, q2, start, logger);
